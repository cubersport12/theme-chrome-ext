...
export const PaletteBlock =  ({ color, name, description, onChangePalette }: { color: string; name: string; description: string; onChangePalette: (palette: Palette) => void }) => {
  ...
  const handleChangeColor: ChangeEventHandler = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    onChangePalette(generateShades(e.target.value));
  }
